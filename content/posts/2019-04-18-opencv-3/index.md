...
利用手机相机对棋盘图拍摄得到的图像如下所示：

<!-- ![](https://i.loli.net/2019/04/18/5cb838359bf08.jpg) -->
{{< figure align=center src="https://i.loli.net/2019/04/18/5cb838359bf08.jpg" caption="图 1 手机相机拍摄图像（左）及其二值化处理后的结果（右）">}}

将其与原始棋盘图对比，可以明显地看出，手机拍摄图像出现了一定程度的畸变。

<!-- ![](https://i.loli.net/2019/04/18/5cb8385ce3115.jpg) -->
{{< figure align=center src="https://i.loli.net/2019/04/18/5cb8385ce3115.jpg" caption="图 2 原始图像（左）与二值化处理后的手机拍摄图像（右）对比">}}
...
<!-- $$
\begin{align}
dx & = x(k_1r^2+k_2r^4+k_3r^6) + p_1(r^2+2x^2) + p_2xy \cr
dy & = \underbrace{y(k_1r^2+k_2r^4+k_3r^6)}_{\text{radial distortion}} + 
    \underbrace{p_2(r^2+2y^2) + p_1 xy}_{\text{tangential distortion}}
\end{align}
$$ -->

$$
\begin{align*}
dx & = \overbrace{x(k_1r^2+k_2r^4+k_3r^6)}^{\text{radial distortion}} + p_1(r^2+2x^2) + p_2xy \cr
dy & = y(k_1r^2+k_2r^4+k_3r^6) + \underbrace{p_2(r^2+2y^2) + p_1 xy}_{\text{tangential distortion}}
\end{align*}
$$
...
$$
\begin{bmatrix}x \cr y \cr f \end{bmatrix}=
K
\begin{bmatrix}X \cr Y \cr Z \end{bmatrix}=
\begin{bmatrix}f_x & 0 & c_x \cr 0 & f_y & c_y \cr 0 & 0 & 1 \end{bmatrix}
\begin{bmatrix}X \cr Y \cr Z \end{bmatrix}
$$
...
{{< figure align=center src="https://s2.ax1x.com/2019/04/19/EpoSpR.jpg" caption="图 3 手机相机在不同角度、位置摄影得到 10 张图像">}}
...
{{< figure align=center src="https://i.loli.net/2019/04/19/5cb96a01bdea5.jpg" caption="图 4 `findChessboardCorners()` 函数在单张相片上的计算结果">}}

不过可能是由于光照等因素影响，第八张相片中的点并未成功识别出。程序将尝试在所有输入图像中找到并绘出这些棋盘点：

<!-- ![DRAW_ALL.jpg](https://i.loli.net/2019/04/19/5cb96b20950d2.jpg) -->
{{< figure align=center src="https://i.loli.net/2019/04/19/5cb96b20950d2.jpg" caption="图 5 全部棋盘点识别结果">}}

最终我们通过 `imwrite()` 函数，输出校正变形后的图像：

<!-- ![EpTKPJ.png](https://s2.ax1x.com/2019/04/19/EpTKPJ.png) -->
{{< figure align=center src="https://s2.ax1x.com/2019/04/19/EpTKPJ.png" caption="图 6 对相机畸变进行校正后的结果">}}
...
\begin{align*}
\text{Camera Matrix} 
&= \begin{bmatrix} f_x & 0   & c_x \cr 
                   0   & f_y & c_y \cr
                   0   & 0   & 1   \end{bmatrix} \cr
&= \begin{bmatrix} 1146.282 & 0        & 450 \cr 
                   0        & 1146.282 & 600 \cr 
                   0        & 0        & 1   \end{bmatrix}
\end{align*}
$$
...
$$
\begin{align*}
\text{distortion coeffients} &= \begin{bmatrix}k_1&k_2&p_1&p_2&k_3\end{bmatrix} \cr
