...
{{< figure align=center src="https://i.loli.net/2019/04/04/5ca60f38390f0.jpg" width="50%" caption="图 1 角点">}}
...
$$
	\begin{cases}
	V_1 = \sum_{i = -k}^{k-1}(g_{c+i,r}-g_{c+i+1, r})^2       \cr
	V_2 = \sum_{i = -k}^{k-1}(g_{c+i,r+i}-g_{c+i+1, r+i+1})^2 \cr 
	V_3 = \sum_{i = -k}^{k-1}(g_{c,r+i}-g_{c+i+1, r+i+1})^2   \cr 
...
{{< figure align=center src="https://i.loli.net/2019/04/04/5ca60af5e8a72.jpg" caption="图 2 Moravec 算子 输出结果">}}
...
$$
\left.\begin{matrix}
g_u = \frac{\partial g}{\partial u} = g_{i + 1, j + 1} - g_{i, j} \cr\cr
...
Q = N^{-1} = 
\begin{bmatrix} 
    \sum g^2_u & \sum g_ug_v \cr
    \sum g_vg_u & \sum g^2_v
\end{bmatrix}^{-1}
$$
...
$$
\begin{align*}
&\sum g_u^2 = \sum_{i = c - k}^{c+k-1} \sum_{j=r-k}^{r+k+1}(g_{i+1, j+1} - g_{i,j})^2 \cr
&\sum g_v^2 = \sum_{i = c - k}^{c+k-1} \sum_{j=r-k}^{r+k+1}(g_{i, j+1} - g_{i+1,j})^2 \cr
...
$$
\begin{align*}
    w &= \frac{1}{\text{tr}(Q)} = \frac{\det(N)}{\text{tr}(N)} \cr
    q &= \frac{4\det(N)}{\text{tr}(N)^2}
\end{align*}
$$
...
**5. 选取极值点。**
以权值 $w$ 为依据选择极值点，即在一个适当窗口中选择最大的待选点，而去掉其余的点。
...
{{< figure align=center src="https://i.loli.net/2019/04/04/5ca60c4b29a90.jpg" caption="图 3 Forstner 算子输出结果">}}
...
$$
\begin{align*}
I(x+u,y+v) &= I(x, y) + I_x(x, y)u + I_y(x, y)v + O(u^2, v^2) \cr
...
$$
M = \sum_{x, y} w(x, y) 
    \begin{bmatrix} I_xI_x & I_xI_y \cr I_xI_y & I_yI_y\end{bmatrix}
= \begin{bmatrix}\sum_{w} I_x^2 & \sum_{w} I_x I_y \cr \sum_{w} I_x I_y & \sum_{w} I_y^2 \end{bmatrix}
= \begin{bmatrix}A & C \cr C & B\end{bmatrix}
$$
...
{{< figure align=center src="https://i.loli.net/2019/04/04/5ca6167f2d040.jpg" width="50%" caption="图 4 椭圆函数">}}
...
{{< figure align=center src="https://i.loli.net/2019/04/04/5ca617f724bbd.jpg" width="50%" caption="图 5 椭圆函数特征值与图像中的角点、直线（边缘）和平面之间的关系">}}
...
$$
\begin{align*}
    \det(M) = \lambda_1\lambda_2 = AB - C^2 \cr
...
$$
\begin{align*}
    I_x = \frac{\partial I}{\partial x} = I \bigotimes (-1 \ 0 \ 1), \cr
...
**3. 使用高斯函数对 $I_x^2$、$I_y^2$ 和 $I_xI_y$ 进行高斯滤波，生成矩阵 $M$ 的元素 $A$、$B$ 和 $C$。**
这里，高斯卷积模板的 $\sigma$ 取 $0.3 \sim 0.9$。
...
{{< figure align=center src="https://i.loli.net/2019/04/04/5ca60b7d85c44.jpg" caption="图 6 Harris 算子输出结果">}}
